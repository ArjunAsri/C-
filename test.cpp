...
TestResult testAddCustomer() {
	...
	ASSERT(customers.size() == 3)
		...
		vector<Customer *> arrayCust;
	arrayCust.push_back(ben);
	arrayCust.push_back(luke);
	arrayCust.push_back(samisoni);

	//Making sure that our getCustomers actually returns the same array
	ASSERT(fms.getCustomers() == arrayCust)
...
TestResult testAddFlight() {
	...
	Flight *flight2 = new Flight(path, airplane2);
	...
	vecFlights.push_back(flight);
	vecFlights.push_back(flight2);

	//Making sure getFlights work
	ASSERT(fms.getFlights() == vecFlights);
